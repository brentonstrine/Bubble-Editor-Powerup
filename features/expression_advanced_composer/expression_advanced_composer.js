...
(function () {
  ...
  function injectTrigger(target) {
    // Check if we've already injected next to this specific element
    if (target.getAttribute('data-cl-has-button')) return;

    console.log("💙❤️ Attempting to inject Advanced Composer button after:", target);
    ...
    target.setAttribute('data-cl-has-button', 'true');
    ...
    setTimeout(() => {
      if (!wrapper.parentElement) {
        ...
        target.removeAttribute('data-cl-has-button');
      }
    }, 500);
  }
